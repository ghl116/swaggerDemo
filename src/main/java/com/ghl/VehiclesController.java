package com.ghl;
import com.ghl.bean.ChangeRentalShopParameter;
import com.ghl.bean.UserParam;
...
public class VehiclesController {
    ...
    public ModelMap changeRentalShop(@RequestBody ChangeRentalShopParameter parameter, UserParam userParam)
            throws Exception {
    ...
    @ApiOperation(value = "ApiModelTest", notes = "ApiModel", response = String.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successful — 请求已完成"),
            @ApiResponse(code = 400, message = "请求中有语法问题，或不能满足请求"),
            @ApiResponse(code = 401, message = "未授权客户机访问数据"),
    ...
    @ResponseBody
    @RequestMapping(value = "apiModelTest", method = {RequestMethod.GET,RequestMethod.PATCH})
    public ModelMap apiModelTest(@RequestBody ChangeRentalShopParameter parameter,@RequestBody UserParam userParam)
            throws Exception {

        logger.info("http://localhost:8501/api/v1/vehicles/apiModelTest | {}", parameter);
        logger.info("## 请求时间：{}", new Date());
        ModelMap map = new ModelMap();
        map.addAttribute("apiModelTest", new Date());
